...
VBO::VBO(uint vert_count, const PointType * v, const NormalType * n, const ColorType * c, QVector<uint> &faces )
{
	vertex_vbo_id = 0;
	Normalvbo_id = 0;
	color_vbo_id = 0;
...
void VBO::update()
{
	if(this->isVBOEnabled)
	{
		...
		if(indices.size() > 0)
		{
			update_ebo(&faces_id, indices.size() * sizeof(int), &indices.front());	// ELEMENT_ARRAY case
		}
	}
...
void VBO::render_regular( bool dynamic /*= false*/, bool isForceColor, Vec4d &forceColor )
{
	...
	if(isForceColor)
	{
		...
		glColor4d(forceColor[0],forceColor[1],forceColor[2],forceColor[3]);
	}
