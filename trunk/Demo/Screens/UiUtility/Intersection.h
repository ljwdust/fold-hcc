...
{
	Point  ab  =  b  -  a; Point ba  =  -ab;
	Point  ac  =  c  -  a; Point ca  =  -ac;
	Point  bc  =  c  -  b; Point cb  =  -bc;
	Point  ap  =  p  -  a; Point pa  =  -ap;
	Point  bp  =  p  -  b; Point pb  =  -bp;
	Point  cp  =  p  -  c; Point pc  =  -cp;
	double  snom  =  dot(ap, ab),  sdenom  =  dot(bp,  ba);
	double  tnom  =  dot(ap, ac),  tdenom  =  dot(cp,  ca);
	//double  snom  =  dot(p  -  a,  ab),  sdenom  =  dot(p  -  b,  a  -  b);
	//double  tnom  =  dot(p  -  a,  ac),  tdenom  =  dot(p  -  c,  a  -  c);
	if  (snom  <=  0.0f  &&  tnom  <=  0.0f)  return  a;   
	//double  unom  =  dot(p  -  b,  c  -  b),  udenom  =  dot(p  -  c,  b  -  c);
	double  unom  =  dot(bp,  bc),  udenom  =  dot(cp,  cb);
	if  (sdenom  <=  0.0f  &&  unom  <=  0.0f)  return  b;   
	if  (tdenom  <=  0.0f  &&  udenom  <=  0.0f)  return  c; 
	Point  n  =  cross(ab,  ac);
	Point  pab = cross(pa, pb);
	double  vc  =  dot(n,  pab);
	//double  vc  =  dot(n,  cross(a  -  p,  b  -  p));

	if  (vc  <=  0.0f  &&  snom  >=  0.0f  &&  sdenom  >=  0.0f)
		return  a  +  snom  /  (snom  +  sdenom)  *  ab;
	Point pbc = cross(pb,  pc);
	double  va  =  dot(n,  pbc);
	//double  va  =  dot(cross(b  -  a,  c  -  a),  cross(b  -  p,  c  -  p));
	if  (va  <=  0.0f  &&  unom  >=  0.0f  &&  udenom  >=  0.0f)
		return  b  +  unom  /  (unom  +  udenom)  *  bc;
	Point pca = cross(pc,  pa);
	double  vb  =  dot(n,  pca);
	//double  vb  =  dot(cross(b  -  a,  c  -  a),  cross(c  -  p,  a  -  p));
