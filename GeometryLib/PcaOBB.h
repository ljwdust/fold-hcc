...
private:
	...
	void build_from_covariance_matrix( Eigen::Matrix3d &C, std::vector<Vec3d> &pnts ){
		...
		for( int i = 0; i < (int)pnts.size(); i++ ){
			...
			minim = minimize( minim, p_prime );
			maxim = maximize( maxim, p_prime );
		}
...
public:
	...
	Box box()
	{
		...
		b.Extent = this->extents();
		b.Axis = QVector<Vec3d>::fromStdVector(axis());
